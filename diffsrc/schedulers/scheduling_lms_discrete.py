...
class LMSDiscreteSchedulerBackOutput(BaseOutput):
    """
    Output class for the scheduler's step function output.
...
class LMSDiscreteScheduler(SchedulerMixin, ConfigMixin):
    ...
    def stepback(
        ...
        order: int = 4,
        return_dict: bool = True,
    ) -> Union[LMSDiscreteSchedulerBackOutput, Tuple]:
        """
        Predict the sample at the previous timestep by reversing the SDE. Core function to propagate the diffusion
        process from the learned model outputs (most often the predicted noise).
        ...
        # 4. Compute previous sample based on the derivatives path
        next_sample = sample - sum(
            coeff * derivative for coeff, derivative in zip(lms_coeffs, self.derivatives)
        )

        if not return_dict:
            return (next_sample,)

        return LMSDiscreteSchedulerBackOutput(next_sample=next_sample, pred_original_sample=pred_original_sample)
